...
& (Join-Path $PSScriptRoot 'Build-StatsR3UnityPackage.ps1') -UnityPath $Unity2022Path -OutputPath $statsR3PackagePath
...
))
{
    ...
    & (Join-Path $PSScriptRoot 'Smoke-Test-StatsR3UnityPackage.ps1') `
        -UnityPath $editor.Path `
        -UnityVersion $editor.Version `
        -StatsR3PackagePath $statsR3PackagePath `
        -StatsPackagePath $statsPackagePath `
        -TagsPackagePath $tagsPackagePath
}

$statsReportPath = Join-Path $repositoryRoot 'artifacts\Klrpxy.Gameplay.Stats.validation.json'
$statsR3ReportPath = Join-Path $repositoryRoot 'artifacts\Klrpxy.Gameplay.Stats.R3.validation.json'
...
& (Join-Path $PSScriptRoot 'Verify-StatsUnityPackage.ps1') `
    ...
    -ReportPath $statsReportPath `
    -EditorVersions $editorVersions
& (Join-Path $PSScriptRoot 'Verify-StatsR3UnityPackage.ps1') `
    -PackagePath $statsR3PackagePath `
    -ReportPath $statsR3ReportPath `
    -EditorVersions $editorVersions

Write-Output "KLRPXY_STATS_R3_UNITY_MATRIX_PASS unity2022=$Unity2022Version unity6=$Unity6Version coreReport=$statsReportPath adapterReport=$statsR3ReportPath"
