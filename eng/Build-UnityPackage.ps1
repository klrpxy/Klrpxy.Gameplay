...
if (-not $OutputPath)
{
    $OutputPath = Join-Path $repositoryRoot 'artifacts\Klrpxy.Gameplay.Tags.0.1.0.unitypackage'
}
...
'@ | Set-Content -LiteralPath (Join-Path $stagingRoot 'ProjectSettings\ProjectVersion.txt') -Encoding utf8
    ...
    $stagedPackage = Join-Path $stagingRoot 'Klrpxy.Gameplay.Tags.0.1.0.unitypackage'
    $process = Start-Process -FilePath $UnityPath -WorkingDirectory $stagingRoot -Wait -PassThru -NoNewWindow -ArgumentList @(
        ...
        'Assets/KlrpxyGameplayTags', 'Klrpxy.Gameplay.Tags.0.1.0.unitypackage')
