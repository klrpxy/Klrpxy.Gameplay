...
生成代码不能直接调用 Runtime 的 internal 构造器。Runtime 在 `StatSet` 上提供受保护的 Key 创建 seam，生成代码通过它创建静态只读 Key；成员描述保存属性路径、种类、类型以及从 StatSet 实例读取成员的 getter，不能在静态元数据中直接引用实例属性。普通玩法代码只在 Group 共享规则中使用生成 Key，并且不接触这条桥接 seam；直接目标规则使用实际 Stat。
