...
Analyzer 自身不依赖 Tags Runtime，并在引用 Stats Runtime 的消费者编译入口检查最低兼容 Tags Runtime。未引用 Stats Runtime 的独立 Unity 程序集不参与该检查；真正缺少依赖时产生带稳定编号和安装指引的错误，使 `.unitypackage` 无法声明包依赖的限制不会退化为难以理解的类型缺失。
