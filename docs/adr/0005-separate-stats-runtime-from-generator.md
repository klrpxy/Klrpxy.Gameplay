...
生成代码不能直接调用 Runtime 的 internal 构造器。Runtime 在 `StatSet` 上提供受保护的 Key 创建 seam，生成代码通过它创建静态只读 Key；成员描述保存属性路径、种类、类型以及从 StatSet 实例读取成员的 getter，不能在静态元数据中直接引用实例属性。普通玩法代码在 Group 共享规则和显式高级输入中使用生成 Key，并且不接触这条桥接 seam；固定值 fluent 直接规则使用实际 Stat。
...
Analyzer 自身不依赖 Tags Runtime，并在引用 Stats Runtime 的消费者编译入口检查最低兼容 Tags Runtime 及 Stats 实际使用的集成类型。未引用 Stats Runtime 的独立 Unity 程序集不参与该检查；真正缺少或二进制不兼容时产生带稳定编号和安装指引的错误，使 `.unitypackage` 无法声明包依赖的限制不会退化为难以理解的类型缺失。
